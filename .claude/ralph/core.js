...
// to close.
const DOC_ONLY_PATH_PATTERNS = [/\.md$/i, /(^|\/)docs\//];
...
//
// The status field is the first two columns and is POSITION-significant: a
// modified tracked file is ` M path` (leading space = clean index). Trimming
// the line before slicing off those columns — as an earlier version did —
// eats the first character of the path instead (` M docs/a.sql` became
// `cs/a.sql`), so every doc-only diff of *modified* files looked like code
// and ran the full review passes for nothing. Found by a unit check during
// the port, 2026-09-17. Slice first, trim after.
//
// A rename/copy entry is `R  old -> new`; only the destination matters here.
...
// a BLOCKED-DB-CHANGE case) well before the real, final verdict. Fixed
