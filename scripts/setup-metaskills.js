 ...
 *
 * Claude Code (и Ralph) ищут skills на глубине .claude/skills/<name>/SKILL.md, а пакет кладёт
 * их в node_modules/metaskills/skills/<name>. Симлинки/junction не подходят: Ralph собирает
 * список skills через entry.isDirectory(), а для ссылок это false, к тому же ссылки не
 * переживают git clone. Поэтому копируем обычные директории; обновление пакета подтягивается
 * на следующем `npm install`. Копии лежат в .gitignore.
 *
